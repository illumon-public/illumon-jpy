...
int JType_MatchVarArgPyArgAsJObjectParam(JNIEnv* jenv, JPy_ParamDescriptor* paramDescriptor, PyObject* pyArg, int idx)
{
    ...
    for (int ii = 0; ii < remaining; ii++) {
        ...
        if (componentType == JPy_JString) {
            matchValue = JType_MatchPyArgAsJStringParam(jenv, paramDescriptor, unpack);
        } else {
            matchValue = JType_MatchPyArgAsJObject(jenv, componentType, unpack);
        }
        if (matchValue == 0) {
            return 0;
        }
...
int JType_MatchPyArgAsJObject(JNIEnv* jenv, JPy_JType* paramType, PyObject* pyArg)
{
    ...
    if (paramComponentType != NULL) {
        ...
        //JPy_DIAG_PRINT(JPy_DIAG_F_TYPE, "JType_MatchPyArgAsJObject: type is an array: %s, primitive=%d\n", paramComponentType->javaName, paramComponentType->isPrimitive);
	    ...
	    PyObject* objectsRepresentation = PyObject_Repr(pyArg);
	    const char* s = PyString_AsString(objectsRepresentation);

	    JPy_DIAG_PRINT(JPy_DIAG_F_TYPE, "JType_MatchPyArgAsJObject: type is an array, sequence is true: %s\n", s);
            return 10;
        }
